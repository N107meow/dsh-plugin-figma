...
- insert:
    - id: figma-mcp
      ...
      config:
        ...
        bridgePort: 8790
        allowWrites: false               # 写操作总闸，默认关（见 §9）
```
...
figma_call({ op, args, target })
  ...
  ├─ 2. target 存在则解析 URL → fileKey/nodeId（与 args 冲突时 args 优先并提示）
  ├─ 3. 参数按 ParamSpec 校验：类型、必填、范围、枚举
  ├─ 4. 破坏性操作（mutating）走 DSH 审批
...
REST 拿不到的、只有 Plugin API 能给的：**当前选区、视口、当前页面、节点写操作、`pluginData`、按 Figma 语义遍历**。
...
- `action: 'run'`（在 Figma 沙箱里执行代码并回传 JSON）是这套桥的杀手锏——**用一次插件往返，换来"任意只读查询"能力**，比在 registry 里穷举几十个 plugin 能力划算得多。写操作不要开自动执行，必须走审批。
...
- 写操作（评论、变量、dev resources）默认关闭，需要显式 config 打开，且逐个走审批；
...
| 阶段 | 内容 | 估时 |
|---|---|---|
| P0 | core 骨架 + 3 spec + 3 工具 + 接线 + 测试 | 2–3 天 |
| P1 | 设计系统 spec + 能力文档生成 | 1–2 天 |
| P2 | MCP 适配器 | 0.5–1 天 |
| P3 | Figma 伴生插件 + 桥 + Client 面板 | 3–4 天 |

P0 结束就已经是一个**能天天用的东西**；P3 是锦上添花。
...
**"写操作"指所有会改变 Figma 云端真实数据的调用**，与只读调用相对：
...
**我为什么默认关掉它**，三个理由，按重要性排序：

1. **读错了只是浪费一次额度，写错了是在改真实的设计资产。** 而且改的是**别人能看到的东西**——评论会通知全团队，改变量会影响所有引用该变量的设计稿。这类操作没有"撤销"按钮，回滚得靠人手工做。
2. **权限一旦给出去，模型犯错的成本就不再由我们控制。** 插件持有的是 PAT，PAT 能做什么完全取决于 token 本身的 scope。默认只请求读 scope，是唯一能保证"模型再怎么幻觉也删不掉东西"的方式。
3. **写操作的额度占用是"隐性"的**：一轮"批量改 40 个变量"的对话，在 Tier 2 的 25/min 下会排队很久，而模型未必知道自己触发了限流。

**如果你说"要"**，我会这样做（不是简单地把开关打开）：

- 每个写能力单独打 `mutating: true` 标记，**在 `figma_capabilities` 里明确标出来**，让模型自己知道哪些调用不可逆；
- 走 DSH 的审批管线（`tools/pre-execute` 的 ask 分支），**逐次人工确认**，不做"一次授权本轮全放行"；
- **强制 dry-run 优先**：第一次调用只回显"将要改变什么"（目标节点/变量 id、字段、旧值→新值），确认后才真正提交；
- 建议你把 PAT 降级为**只读 token**，写操作走另一个单独的 token 引用（`FIGMA_WRITE_TOKEN`），两个桶、两套 scope，物理隔离。

**我的建议：P0–P1 先不写。** 只读版本已经覆盖"读懂设计"这个主诉求；等它跑顺了、你对模型的行为有把握了，再单独开写操作这一期（估 +1.5 天）。你现在只需要回我一句"先不写"或"要写"，我就按对应路径开工。
