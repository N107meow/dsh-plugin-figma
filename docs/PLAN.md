...
>
> 状态：**设计定稿，等待开工指令**。包名 `dsh-figma-plugin`、交付形态 A only、只读、npm + GitHub 双通道分发——全部已定（§9）。P0 范围已冻结（§9.1.3）。文中所有关于 DSH 内部接口的结论，均已在本机部署上实测核对，出处标注在 §10。
>
> **项目将开源，但范围是 DeepSeek Harness 插件**（受众 = 其他 DSH 用户）。由此产生的约束见 §12：主要是**默认值要面向更弱的 Figma 席位**，以及配置/排障的易用性——**不含多宿主分发**。
>
> **交付形态已确认：仅 A**（DSH 原生 Cordis 插件，P0 交付）。MCP 适配器（B）当前不做，但保留了「可随时补回」的 CI 可检查不变量（§12.3.1）。**分发：npm + GitHub 双通道**（§12.9.1）。包名 `dsh-figma-plugin`（§9.1.2）。
...
```
dsh-figma-plugin/                 # 仓库根 = 包根（GitHub 分发要求，§12.9.1）
├── package.json                  # name: "dsh-figma-plugin"；main → lib/index.js
├── src/
...
### 9.1.2 包名：**`dsh-figma-plugin`** ✅ 已定

最初定为 `dsh-figma`。**实测 npm registry 后发现该名已被占用**，故改用 `dsh-figma-plugin`。以下保留冲突证据以备将来查阅：
...
**最终选择：`dsh-figma-plugin`**（已实测在 npm 上可用）。命名演变为 `dsh-figma`（被占）→ `dsh-figma-plugin` → **`dsh-figma-plugin`**，定稿理由是它最清楚地分段表达"**dsh** 的 **figma** **plugin**"：读一遍就知道是 DSH 的 Figma 插件。无 scope 形式比 `@scope/...` 更短、更好念。

**命名统一表（这是唯一权威）**：

| 位置 | 值 | 说明 |
|---|---|---|
| npm `package.json` 的 `name` | **`dsh-figma-plugin`** | 唯一标识，安装与发布都用它 |
| 仓库名 / 本地目录名 | `dsh-figma-plugin` | 与包名一致，避免两套名字 |
| `cordis.patch.yml` 的 `name` 字段 | **`dsh-figma-plugin`** | loader 按此解析包（§12.9.1 实测） |
...
**接线**：`~/.dsh/profiles/web/` 的 `package.json` 加依赖 + `cordis.patch.yml` insert 一行。**依赖名与 `package.json` 的 `name` 均为 `dsh-figma-plugin`**；`cordis.patch.yml` 里 `name: 'dsh-figma-plugin'`、`id: figma`（§9.1.2 命名统一表）。
...
- **命名**：✅ 已定 **`dsh-figma-plugin`**（§9.1.2）——描述性命名，不宣称官方归属；README 明确"非 Figma 官方项目，未获 Figma 背书"。
...
```bash
dsh plugin --profile web add dsh-figma-plugin     # 等价于在 profile 目录里 pnpm add
```
...
```bash
pnpm add github:<user>/dsh-figma-plugin
```
...
- 由你来验（你的网络显然更好——实测你能正常访问 npm registry：1.7s 返回 200）：`pnpm add github:<user>/dsh-figma-plugin` 能否成功；
...
```
dsh-figma-plugin/               # 仓库根 = 包根，name: "dsh-figma-plugin"
...
- [ ] 包名 `dsh-figma-plugin`，README 标注"非 Figma 官方项目，未获 Figma 背书"
...
- [ ] **改名守卫**：`package.json` 的 `name` 是唯一权威；改名时用 `grep -rn '<旧名>' . --exclude-dir=.git` 收尾，**必须为 0 命中**。本次 `dsh-plugin-figma` → `dsh-figma-plugin` 时，`repository` 之外的 `bugs` / `homepage` 两个 URL 就漏改过（自查脚本抓到的）
