...
>
> 已确定的决策：包名 **`dsh-figma-plugin`** · 交付形态 **仅 DSH 原生 Cordis 插件**（MCP 适配器当前不做，但保留了可补回的 CI 不变量）· **只读**，不做任何写操作 · **npm + GitHub 双通道分发**。
>
...
> 注：最初选的 `dsh-figma` 在 npm 上已被他人占用（`dushaobindoudou`，2026-08-19 发布 `0.0.1` 占位版，做的是同类项目），故改用 **`dsh-figma-plugin`**。详见 `docs/PLAN.md` §9.1.2。

**通道二 · GitHub（不占 npm 名）**

```bash
cd ~/.dsh/profiles/web && pnpm add github:<user>/dsh-figma-plugin
```
...
```bash
git clone <repo> ~/dsh-figma-plugin
cd ~/.dsh/profiles/web && pnpm add link:~/dsh-figma-plugin
```
